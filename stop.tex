...
\eq{bkgEstLL}
{
N^{Data_{Est},~SR}_{lost~\ell}  =  N^{Data,~CR,~E_{T}^{miss}~ext.}_{2\ell} \times  \frac{N^{MC,~SR,~E_{T}^{miss}~ext.}_{lost~\ell}}{ N^{MC,~CR,~E_{T}^{miss}~ext.}_{2\ell}} \times \frac{ N^{MC,~SR}_{lost~\ell}}{N^{MC,~SR,~E_{T}^{miss}~ext.}_{lost~\ell}},
}
