...
\chapterwithnum{Search for top squark pair production in pp collisions at $\sqrt{s}$=13~TeV in Run 2 using single lepton events}
\setcounter{secnumdepth}{4}
...
%Beyond 1 Tev - why is that possible
As mentioned previously, the Ref.~\cite{Baer:2016bwh} shows that the mass of the lighter stop $(\tilde{t}_{1})$ can be up to the 3~TeV can lead to the natural SUSY. This reference discusses that if the obesrvable $O$ can be written as
...
%Beyond the studied model
%compressed arXiv:1504.00504, dispalced signatures: arXiv:1601.01326 not all studied at LHC
% Matural SUSY in unconventional signatures -?Mariotti


%reinterpretations
%pssm https://arxiv.org/pdf/1606.03577.pdf
	
%Beyond mssm - RPV susy
%normally r-parity assumed in order not  to have proton decay
% but there are proposals how to violate parity and make proton stable
%for example replace r-parity by a new symmetry, which conserves baryon but violates lepton number -> this symmetry allows LSP to decay
%then collider signatures completely different
	%both pair and single sparticle production
	%LSP can decay - if lepton number violation neutralino decays to two opposite charge (maybe even difefrent flavour) leptons plus neutrino -> charged leptons or large MET or both
	%if B violation LSP decaying to hadrons
	%the signal signature depends on the r-parity violation model and therefore it is different for different models
	%also in some cases, other sparticles other than LSP can decay to to SM particles
		%several models already excluded at LHC


%compressed spectra

%dark matter motivation for SUSY (Martin 129)
%known cold dark matter density $\Omega_{DM}h^{2} \approx 0.120$ -> omega is the average energy density in non-baryonic dm divide3d by the critical density that would lead to a spatially flat homogenous universe, h is the hubble constant in units of 100 $km sec^{-1} MPc^{-1}$, $h^2$ observed to be 0.46 therefore cold dark matter denisty $\rho_{DM} = 1.2 \times 10^{-6} GeV/cm^{3}$
%LSP the dm candidate - it can be gravitino, sneutralino or lightest neutralino
%sneutrino lsp is largely ruled out by  direct searches [284]
%gravitions possible candidate which could have the correct relic density but impossible to detect directly -> their interaction is too weak
%best option for detection is neutralino
% in early universe sparticles were in thermal equilibrum with sm particles, but then universe cooled down,  the heavier particles could not be produced anymore, they anihilated into SM particles or decayed to LSP.
%eventually the anihilation rate became was smaller compared to expansion and there was a freezeout of lsps to the today density (diluted by the expansion)
%IF
%LSP is mostly higgsino or mostly wino, the LSP needs to be very heavy (around 1 TeV) to obtain correct dm density -> this is against hierarchy but within LHC limits
	%to get lighter higgsino-like or wino-like LSP, non-thermal mechanism exist to predict correct density
%IF
%In many susy models lightest neutralino is bino-like
	%but the masses predicting correct dm density are already excluded by collider experiments
	%but the density can be reduced, if at least one of the sfermions is not too heavy -> usually spleptons, but the sleptons are light enough to be already excluded at LHC
	%but there is a viable option where stop is less than 150GeV heavier than LSP, then LSP is between mass of the top and top+100GeV. But it violates the prediction for unification at higher scales - actually now I see than in my analysis it is also excluded
	%or there must be some anihilation mechanism which decreases the abundance of LSP
%MSUGRA is giving masses of squarks to be several TeVs -> attractive

%but simple thermal (anihilation and decay) mechanism to compute the relic density, but does not have to be true, there might be -> e.g. breaking of r-parity, non-thermal mechanism ( such as out-of-equilibrum decays), or non-thermal production of neutralinos after freeze-out
	%therefore the assumptions above just for simplified model and it is not strict exclusion 

%Some ideas

%SM measuremets of coupling -> bsm physics
%b-> s gamma measurements

%Other possibilities beyond MSSM -NMSSM
