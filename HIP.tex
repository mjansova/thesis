...
\eq{fakeEstimation}
{
   p_{fake} = p_{HIP}(PU)\times T \times p(track)/PU  \times N_{fake} \times PU^2 \times  \frac{\sigma_{position}}{N_{strips} \times pitch}  ,
}

where the time window $T$ is the number of bunch crossings in one orbit for which the fake clusters are observed, and the position resolution $\sigma_{position}$ is the estimate of the space window around the reconstructed track in which the fake cluster is close enough to be used for the track reconstruction to the total space window of one APV. The $N_{HIP}$ is the average fake multiplicity over the time window $T$. A cluster is used for the track reconstruction if it passes a certain charge threshold, which depends on the thickness of the sensor and the track inclination. Considering a normal track, the cluster charge cut for the sensors of the first layer of the TOB, which have thickness of 500~$\mu$m, is around 140~ADC in the case of the tightest cut. As shown inthe left plot of Fig.~\ref{fig:figures/avClusterMultiplicitySecondT}, in average all fake clusters pass this requirement. Using the Eq.~\ref{eq:fakeEstimation}, with $f_{HIP} = 0.0024$, $T = 18$, $N_{fake} =0.087 $  and $\sigma_{position} = 2 \sqrt{2} 40 \mathrm{\mu m}$ divided by the number of strips in APV times the pitch, the lower limit on the probability to have at least one fake cluster in the first layer of the TOB after the HIP event has been calculated to be  of order of $0.002\%$. This probability is fortunately very low, but is just a lower limit as there are only 7 consecutive events and the behavior of fake clusters after these events is unknown. %TODO APE, CPE and computation of fake per track, NStrips, pitch
