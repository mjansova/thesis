...
The probability to have a HIP event given by Eq.~\ref{eq:HIPprob2}, is in this study estimated in average separately for each layer of the silicon strip tracker.  The $f_{HIP}$ is calculated according to Eq.~\ref{eq:HIPfrac} and the the number of tracks traversing given tracker APV is computed from the simulated events. The final result can be seen in the left plot of Fig.~\ref{fig:figures/fracHIP2T}. The probability of a HIP event is of order of $10^{-3}-10^{-2}$, which is around an order of magnitude larger than the results provided by the HIP study at the PSI described in section~\ref{sec:ProbPast}. In case of this study, the simulated number of tracks travsersing the tracker modules does not take into account neutral particles, becuse they do not ionize the sensor and thus do not leave hits. Then the probability of the HIP event is thus increased by not taking into account all particles. Also the two analyses differ by the HIP selection and trigger conditions.

In the left plot of Fig.~\ref{fig:figures/fracHIP2T} it can be observed that the probability of a HIP event is different for different layers. The probability increases with the module thickness as the path of the particle in thicker modules is longer. The change of probability because of larger thickness can be typically seen when comparing TIB modules which are 320~$\mathrm{mu m}$ wide and TOB modules which have thickness of 500~$\mu m$. A HIP event probability increases also with the module pitch, larger the pitch is, less channels read the HIP charge and thus it is easier to reach the zero-light level. The probability of the HIP event between layers also change because of the particle and particle energy spectra differ with the distance from the interaction point. The charged particles are bent in magnetic field and thus only highly energetic charged particles reach the outer layers of tracker, what is not the case for the neutral particles. Because of this the particle energy spectra as well as the ratio of charged and neutral particles with the distance of the interaction point. The inelastic cross section of the nuclear interaction chnages with the kind of particle and particle energy and therefore by this the probability of the HIP event is changed for different layers. As the fraction of charged to neutrals particles change with a distance from an interaction point the HIP probability has to change betwwen layers. 

The estimation of the HIP events probability which is not sensitive to the tracking is defined in Eq.~\ref{eq:HIPprob2}. This probability is rescaled to the fill pile-up and thus should be invariant with change of the fill structure. But it is not completely true, because the out-of-time pile-up can lead to loopers which cause the HIP event, but as in the run 281604 there is no out-of-time pile-up, the loopers are not taken into account. However the $f_{HIP}$ rescaled by the fill pile-up of $47.881$ shown in right plot of Fig.~\ref{fig:figures/fracHIP2T} can serve as a lower limit of the HIP probability, which was computed to be of the order of~$10^{-6}-10^{-5}$. This probability changes from layer to layer because of the differences in module geometry and particle composition and energy spectra, but the relative change of probability betwween gioven layers for the different fills should remain the same.

Moreover as previously, bot computed probabilities of the HIP events can be biased by the trigger conditions. Only baselines which are already fully suppressed in the first event or which are still fully suppressed in the second event~(after 75~ns) can be selected as HIP events, what is potentially decreasing the probability. On the other hand baseline can remain saturated for more bunch crossings and thus one HIP event can be selected in more consecutive bunch crossings and as consequence the probability can be increased.

    \insertTwoFigures{figures/fracHIP2T}
                 {figures/fracHIP2} % Filename = label
                 {figures/fracHIP3} % Filename = label
                 {0.47}       % Width, in fraction of the whole page width
                 {An average probability of the HIP event per layers of silicon strip tracker, computed from data run 281604.} 
