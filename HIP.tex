...
where $N_{HIP}$ is number of selected HIP events, $N_{all}$ is number of all events, is computed for all partitions and layers of the tracker.
...
-VR data
-iterated median or normal median - how is this computed?
-general strategy: low baselines?
-limitations - trigger rules, a lot of data
-combining and ordering of events
-mention teh code I started from?
-TOB L1
...
\subsubsection{Experimental setup} 
            ( trigger and filling scheme, closely spaced events, VR, studies after HIP, [TOBl1], trigger rules, statistical study)


In this analysis, closely spaced events were enriched by using special trigger cnofiguration, which forced first bunch crossing in fixed train to be triggered. After this trigger two other bunch crossings in the same train were triggered randomly. The final number of triggered events as function of bunch crossing is shown in figure~\ref{fig:figure\triggerStudy1}. The shape of the trigger distribution is given by trigger rules as well as forced trigger on first bx in the train.


-run 273162
-fill
-HIP selection: low baseline vs low rms
-align HIP with BX=0
-study the HIP before and after BX=0
-for plots only 1st layer of tob used, whci was predicted to be moce affcted, but for $f_HIP$ all layers of traker
...
-in which BX HIP selected - says something about duration of full saturation
-14 – 20 MB VR event size compared to 450-570 kB ZS of standard data.


 \subsubsection{Methodology}
 
          (selection (or separate chapter), baseline, cluster charge & multiplicity)

-selection: baseline < -5 ADC, RMS of raw digis < 2.5
-baseline distribution - evolution, baseline recovery is not the same as cluster recovery  maybe some picture of Rob
